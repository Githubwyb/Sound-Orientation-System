...
void adc2_init(void)
{
    ...
    //                     设置A通道的负输入 | 设置A通道的正输入         |  设置B通道的负输入          | 设置B通道的正输入
    SetChanADC10( ADC_CH0_NEG_SAMPLEA_NVREF | ADC_CH0_POS_SAMPLEA_AN9 |  ADC_CH0_NEG_SAMPLEB_NVREF | ADC_CH0_POS_SAMPLEB_AN5); // configure to sample AN4 & AN5
    ...
    //开启读取定时器
    TIMER_RequestTick(adc2_getData_handler, 10);
